...
const Nut = preload("res://items/nut.tscn")
	...
	match state:
		...
		CHASE:
			...
			if player != null:
				...
				if wanderController.get_time_left() == 0:
					update_wander([BASH, WANDER])
			else:
				state = IDLE
		BASH:
			var player = playerDetectionZone.player
			if player != null:
				accelerate_towards_point(player.global_position, delta, bash_speed)
				$AnimatedSprite.play("bash")
				if wanderController.get_time_left() == 0:
					update_wander([CHASE, WANDER, IDLE])
			else:
				state = IDLE
